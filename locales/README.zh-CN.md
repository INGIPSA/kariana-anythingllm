...
<p align="center">
    <b>Kariana：</b> 您一直在寻找的全方位AI应用程序。<br />
...
<p align="center">
👉 适用于桌面（Mac、Windows和Linux）的Kariana！<a href="https://kariana.com/download" target="_blank">立即下载</a>
</p>

这是一个全栈应用程序，可以将任何文档、资源（如网址链接、音频、视频）或内容片段转换为上下文，以便任何大语言模型（LLM）在聊天期间作为参考使用。此应用程序允许您选择使用哪个LLM或向量数据库，同时支持多用户管理并设置不同权限。

![聊天](https://github.com/INGIPSA/kariana/assets/16845892/cfc5f47c-bd91-4067-986c-f3f49621a859)
...
Kariana是一个全栈应用程序，您可以使用现成的商业大语言模型或流行的开源大语言模型，再结合向量数据库解决方案构建一个私有ChatGPT，不再受制于人：您可以本地运行，也可以远程托管，并能够与您提供的任何文档智能聊天。

Kariana将您的文档划分为称为`workspaces` (工作区)的对象。工作区的功能类似于线程，同时增加了文档的容器化。工作区可以共享文档，但工作区之间的内容不会互相干扰或污染，因此您可以保持每个工作区的上下文清晰。

## Kariana的一些酷炫特性

- 🆕 [**完全兼容 MCP**](https://docs.kariana.com/mcp-compatibility/overview)
- 🆕 [**无代码AI代理构建器**](https://docs.kariana.com/agent-flows/overview)
- 🖼️ **多用户实例支持和权限管理（支持封闭源和开源LLM！）**
- [**自定义人工智能代理**](https://docs.kariana.com/agent/custom/introduction)
- 👤 多用户实例支持和权限管理 _仅限Docker版本_
- 🦾 工作区内的智能体（浏览网页、运行代码等）
- 💬 [为您的网站定制的可嵌入聊天窗口](https://github.com/INGIPSA/kariana-embed/blob/main/README.md)
...
- [Novita AI (聊天模型)](https://novita.ai/model-api/product/llm-api?utm_source=github_kariana&utm_medium=github_readme&utm_campaign=link)
- [PPIO (聊天模型)](https://ppinfra.com?utm_source=github_kariana)
...
- [Kariana原生嵌入器](/server/storage/models/README.md)（默认）
- [OpenAI](https://openai.com)
- [Azure OpenAI](https://azure.microsoft.com/en-us/products/ai-services/openai-service)
...
- [Kariana内置](https://github.com/INGIPSA/kariana/tree/master/server/storage/models#audiovideo-transcription) （默认）
- [OpenAI](https://openai.com/)
...
- `embed`: 用于生成和创建[网页嵌入组件](https://github.com/INGIPSA/kariana-embed)的子模块.
- `browser-extension`: 用于[Chrome 浏览器扩展](https://github.com/INGIPSA/kariana-extension)的子模块.

## 🛳 自托管

INGIPSA和社区维护了许多部署方法、脚本和模板，您可以使用它们在本地运行Kariana。请参阅下面的表格，了解如何在您喜欢的环境上部署，或自动部署。
...
[其他方案：不使用Docker配置Kariana实例 →](../BARE_METAL.md)
...
_以下是一些与 Kariana 兼容的应用程序，但并非由 INGIPSA 维护。列在此处并不代表官方背书。_

- [Midori AI 子系统管理器 - 使用 Docker 容器技术高效部署 AI 系统的简化方式](https://io.midori-ai.xyz/subsystem/kariana/) - 使用 Docker 容器技术高效部署 AI 系统的简化方式。
- [Coolify](https://coolify.io/docs/services/kariana/) - 一键部署 Kariana。
- [适用于 Microsoft Word 的 GPTLocalhost](https://gptlocalhost.com/demo/) - 一个本地 Word 插件，让你可以在 Microsoft Word 中使用 Kariana。

## 远程信息收集与隐私保护

由 INGIPSA Inc 开发的 Kariana 包含一个收集匿名使用信息的 Telemetry 功能。

<details>
<summary><kbd>有关 Kariana 的远程信息收集与隐私保护更多信息</kbd></summary>

### 为什么收集信息？

我们使用这些信息来帮助我们理解 Kariana 的使用情况，帮助我们确定新功能和错误修复的优先级，并帮助我们提高 Kariana 的性能和稳定性。
...
[在源代码中查看所有信息收集活动](https://github.com/search?q=repo%3AINGIPSA%2Fkariana%20.sendTelemetry(&type=code)
...
[![kariana 的贡献者们](https://contrib.rocks/image?repo=mintplex-labs/kariana)](https://github.com/mintplex-labs/kariana/graphs/contributors)

[![Star 历史图](https://api.star-history.com/svg?repos=mintplex-labs/kariana&type=Timeline)](https://star-history.com/#mintplex-labs/kariana&Date)
...
版权所有 © 2026 [INGIPSA][profile-link]。<br />
本项目采用[MIT](https://github.com/INGIPSA/kariana/blob/master/LICENSE)许可证。
