# How to use Dockerized Kariana

Use the Dockerized version of Kariana for a much faster and complete startup of Kariana.

### Minimum Requirements

> [!TIP]
> Running Kariana on AWS/GCP/Azure?
...
\*Kariana by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*Kariana by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized Kariana!
...
> the docker container using Kariana as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull ingipsa/kariana
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run Kariana in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/kariana && \
...
$env:STORAGE_LOCATION="$HOME\Documents\kariana"; `
...
services:
  kariana:
    image: ingipsa/kariana
    container_name: kariana
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - kariana_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using Kariana! All your data and progress will persist between
...
- `git clone` this repo and `cd kariana` to get to the root directory.
- `touch server/storage/kariana.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of Kariana easier.

### Use the Midori AI Subsystem to Manage Kariana

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the Kariana docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in Kariana.
