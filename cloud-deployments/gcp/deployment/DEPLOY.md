...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create kariana-deployment --config gcp/deployment/gcp_deploy_anything_llm.yaml
...
```
gcloud compute instances get-serial-port-output kariana-instance 
```

ssh into the instance

```
gcloud compute ssh kariana-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete kariana-deployment 
```
